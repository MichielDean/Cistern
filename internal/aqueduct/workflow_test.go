...
func TestGenerateCataractaeFiles_WithPersonaAndInstructions(t *testing.T) {
	...
	content, err := os.ReadFile(filepath.Join(identityDir, "AGENTS.md"))
	if err != nil {
		t.Fatal(err)
	}
	got := string(content)
...
func TestGenerateCataractaeFiles_SkipsWhenInstructionsMissing(t *testing.T) {
	...
	if _, statErr := os.Stat(filepath.Join(identityDir, "AGENTS.md")); statErr == nil {
		t.Error("AGENTS.md should not be created when INSTRUCTIONS.md is missing")
	}
}
...
func TestGenerateCataractaeFiles_DeduplicatesIdentities(t *testing.T) {
	...
	// Deduplication: 1 identity → 2 files (AGENTS.md + PIPELINE_POSITION.md).
	if len(written) != 2 {
...
// non-default instructionsFile is written instead of AGENTS.md.
func TestGenerateCataractaeFiles_WritesProviderInstructionsFile(t *testing.T) {
	...
	w := workflowWithIdentity("implementer")
	written, err := GenerateCataractaeFiles(w, tmpDir, "CUSTOM.md")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	// Expect 2 files: CUSTOM.md + PIPELINE_POSITION.md.
	if len(written) != 2 {
		t.Fatalf("expected 2 files written, got %d: %v", len(written), written)
	}
	if written[0] != filepath.Join(identityDir, "CUSTOM.md") {
		t.Errorf("written[0] = %q, want CUSTOM.md path", written[0])
	}

	// CUSTOM.md must exist with expected content.
	data, err := os.ReadFile(filepath.Join(identityDir, "CUSTOM.md"))
	if err != nil {
		t.Fatalf("CUSTOM.md not created: %v", err)
	}
	if !strings.Contains(string(data), "I implement.") {
		t.Error("CUSTOM.md missing persona content")
	}

	// AGENTS.md must NOT be created.
	if _, statErr := os.Stat(filepath.Join(identityDir, "AGENTS.md")); statErr == nil {
		t.Error("AGENTS.md should not be created when instructionsFile is CUSTOM.md")
	}
}

// TestGenerateCataractaeFiles_EmptyInstructionsFileDefaultsToOpencode verifies that
// passing "" for instructionsFile defaults to writing AGENTS.md.
func TestGenerateCataractaeFiles_EmptyInstructionsFileDefaultsToOpencode(t *testing.T) {
	tmpDir := t.TempDir()
	identityDir := filepath.Join(tmpDir, "qa")
	if err := os.MkdirAll(identityDir, 0o755); err != nil {
		t.Fatal(err)
	...
	if _, statErr := os.Stat(filepath.Join(identityDir, "AGENTS.md")); statErr != nil {
		t.Error("empty instructionsFile should default to writing AGENTS.md")
	}
}
